...
def multi_query_retrieve(query: str, chunks: list[str], metadatas: list[dict], bm25_index: dict, top_k: int = 3):
    """Expand the query into several phrasings, retrieve with each, and fuse all rankings via RRF."""
    # Indexing the corpus is independent of the query, so this only needs to
    # happen once regardless of how many phrasings we search with.
    ...
    for q in queries:
        ...
        rankings.append(vector_search(query_embedding, top_k=len(chunks)))
        rankings.append([i for i, _ in bm25_search(q, bm25_index)])
