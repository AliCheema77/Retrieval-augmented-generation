...
def cosine_similarity(a, b):
    ...
    differ. Returns a float between -1 and 1 (1 = identical meaning,
    0 = unrelated, -1 = opposite meaning).
    """
    dot_product = np.dot(a, b)
    norm_a = np.linalg.norm(a)
    norm_b = np.linalg.norm(b)
    # Dividing by both magnitudes cancels out vector length, leaving only
    # the directional (angular) similarity.
...
def reciprocal_rank_fusion(rankings: list[list[int]], k: int = 60) -> list[tuple[int, float]]:
    ...
    ranking by each item's *position*, not its raw score — this avoids
    having to normalize incompatible score scales (vector similarity is
    roughly 0-1, BM25 is unbounded and corpus-dependent). `k` is a
    smoothing constant that limits how much any single ranking can
    dominate the fused result.
    """
    fused_scores = {}
    for ranking in rankings:
        for rank, doc_id in enumerate(ranking):
            # Each ranking contributes 1 / (k + rank + 1) to a chunk's
            # total score — a better (lower) rank contributes more.
