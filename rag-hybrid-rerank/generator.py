from pathlib import Path
import ollama

from chunker import load_text, chunk_text
from embedder import embed_chunks
from retriever import hybrid_retrieve
from bm25 import build_bm25_index
from reranker import rerank

SCRIPT_DIR = Path(__file__).parent

# Must already be pulled locally via `ollama pull llama3.2:3b`.
OLLAMA_MODEL = "llama3.2:3b"

SYSTEM_PROMPT = (
    "You are a helpful assistant. Answer the user's question using ONLY the "
    "context provided below. If the answer isn't in the context, say you "
    "don't know — do not make anything up."
)


def build_prompt(query: str, retrieved_chunks: list[tuple[str, float]]) -> str:
    """Assemble the retrieved chunks and the user's question into a single prompt for the LLM."""
    context = "\n\n---\n\n".join(chunk for chunk, _score in retrieved_chunks)
    return f"Context:\n{context}\n\nQuestion: {query}"


def generate_answer(query: str, retrieved_chunks: list[tuple[str, float]]) -> str:
    """Ask the local LLM (via Ollama) to answer the query using only the retrieved chunks as context."""
    prompt = build_prompt(query, retrieved_chunks)

    # Talks to the local Ollama server (localhost:11434) — nothing leaves the machine.
    response = ollama.generate(
        model=OLLAMA_MODEL,
        system=SYSTEM_PROMPT,
        prompt=prompt,
    )

    return response["response"]


if __name__ == "__main__":
    sample_path = SCRIPT_DIR / "sample.txt"
    text = load_text(sample_path)
    chunks = chunk_text(text)
    chunk_embeddings = embed_chunks(chunks)

    query = input(f"Ask your questions here\n")
    bm25_index = build_bm25_index(chunks)

    # Cast a wide net with cheap hybrid retrieval (15 candidates), then let
    # the slower, more accurate cross-encoder narrow it down to the best 3.
    candidate_chunks = hybrid_retrieve(query, chunks, chunk_embeddings, bm25_index, top_k=15)
    retrieved_chunks = rerank(query, candidate_chunks, top_k=3)

    answer = generate_answer(query, retrieved_chunks)

    print(f"Query: {query}\n")
    print(f"Answer:\n{answer}")
